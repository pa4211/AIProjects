...
# Prepare and send the request
            ...
            if 'text' in response_data:
                    ...
                    st.write("Error: No transcription text found in the response.")
                    return None
        else:
                    st.write("Error: Failed to download the audio file.")
                    return None

    except requests.exceptions.RequestException as e:
        st.write(f"Error sending request: {str(e)}")
    except Exception as e:
        st.write(f"Error transcribing {file_name}: {str(e)}")
...
def main():
    ...
    if st.button("Click Me"):
        ...
        #st.write(text)
        # prompt = f"""
        #    Use the {text} and get the top 5 key takeaways from the video in a short summary of bullet points"
        #     """
